...
# 이렇게 해야 매번 새로 다운로드하는 것을 방지할 수 있습니다.
    ...
    try:
        ...
        if "playwright install" in error_msg:
            ...
            try:
                ...
                # PyInstaller에서는 driver_executable이 (node_exe, cli_js) 튜플로 반환될 수 있으므로 분기 처리
                cmd = []
...
def open_naver():
    ...
    if getattr(sys, 'frozen', False):
        base_dir = os.path.join(os.environ.get("LOCALAPPDATA", os.path.join(os.environ.get("USERPROFILE", ""), "AppData", "Local")), "Naver_Open")
        os.makedirs(base_dir, exist_ok=True)
    else:
        base_dir = os.path.dirname(os.path.abspath(__file__))
    
    user_data_dir = os.path.join(base_dir, "naver_user_data")
    ...
    with sync_playwright() as p:
        ...
        print("네이버(https://naver.com)로 이동합니다...")
